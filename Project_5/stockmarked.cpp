...
vec StockMarked::Model(int Nagents, int transactions, double m0, vec agents, double lmbd, double alpha, double gamma)
{
    ...
    //cout << count << endl;
    cout << "Number of transactions made: " << count_transactions << "\n";
    return agents;
}

void StockMarked::Simulation(int Nagents, int runs, int transactions, double m0, string filename, double lmbd, double alpha, double gamma)
{
    ...
    for (int run=0; run<runs; run++){
        ...
        agents = Model(Nagents, transactions, m0, in_agents, lmbd, alpha, gamma);
